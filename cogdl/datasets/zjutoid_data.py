# 如果加上github之外的数据集或者修正github数据集的下载路径，需1、2
# 修改read_zjutoid_data函数下的文件路径，去掉"/github"
# 需要去掉以下重写zjutoid的raw_path方法
...
#         print("folder:"+str(folder))
        # 添加github数据集之外的数据集时，需修改文件路径，去掉"/github"
        with open("{}/github/ind.{}.{}".format(folder,prefix,item), "rb") as f:
            if sys.version_info > (3, 0):
                objects.append(pkl.load(f))
            else:
                objects.append(pkl.load(f))
      # 添加github数据集之外的数据集时，需修改文件路径，去掉"/github"
    test_index = parse_index_file(f"{folder}/github/ind.{prefix}.{names[-1]}")
...
class zjutoid(Dataset):
    url = "https://cloud.tsinghua.edu.cn/f/4fefa24ff5704db9bad1/?dl=1"
    def __init__(self, root, name, split="public", num_train_per_class=20, num_val=500, num_test=1000):
        self.name = name

        super(zjutoid, self).__init__(root)
...
#         return ["ind.{}.{}".format("github", name) for name in names]
    ...
    def __len__(self):
        return 1
# 如果加上github之外的数据集或者修正github数据集的下载路径，需要去掉以下重写的raw_path方法
    @property
    def raw_paths(self):
        r"""The filepaths to find in order to skip the download."""
        x=self.raw_file_names
        if not isinstance(x, collections.Iterable) or isinstance(x, str):
            x = [x]
        files = x
        return [osp.join(osp.join(self.raw_dir,'github'), f) for f in files]
...
@register_dataset("github")
class GithubDataset(zjutoid):
    def __init__(self, data_path="data"):
        dataset = "Github"
        path = osp.join(data_path, dataset)
#         print(path)
        if not osp.exists(path):
            zjutoid(path, dataset)
        super(GithubDataset, self).__init__(path, dataset)
